<template>
    ...
    <button class="main-filter-btn" :v-model="where" @click="choseSearch('where')">{{where}}</button>
    <div class="border-line-between" @click="choseSearch('where')"></div>
    <button class="main-filter-btn" :v-model="when" @click="choseSearch('when-start')">{{when}}</button>
    <div class="border-line-between" @click="choseSearch('when-start')"></div>
    <button class="main-filter-btn guest" :v-model="who" @click="choseSearch('who')">{{who}}</button>
...
import { eventBus } from '../services/event-bus.service'
    export default {
        ...
        created() {
            eventBus.on('indicates-for-main-filter', this.putIndicates)
        ...
        computed: {
            // isWhere(){
            //     return this.$store.getters.filterBy.txt || 'Anywhere'
            // },
            // isWhen(){
            //     return this.$store.getters.filterBy.checkin || 'Any week'
            // },
            // isWho(){
            //     return this.$store.getters.filterBy.adults || 'Add guests'
            // }
        },
    }
</script>
